...
const MyListCard = ({ spot, handleDeleteSpot }) => {

    const { _id, image, country_Name, tourist_spot_name, location, shortdescription } = spot;

    return (
        <div data-aos="zoom-in" data-aos-duration="1000">
            <div className="flex flex-col h-[550px] justify-between p-6 rounded-md shadow-md dark:bg-gray-50 dark:text-gray-900 border border-[#C6EBC5] relative">
                <div>
                ...
                <div className='absolute top-8 right-8 flex flex-col gap-5'>
                    <button onClick={() => handleDeleteSpot(_id)} className='btn text-xl bg-[#FA7070] border-none text-white bg-opacity-80 hover:bg-[#FA7070]'><MdDelete /></button>
                    <Link to={`/update-spot/${_id}`} className='btn text-xl bg-[#A1C398] border-none text-white bg-opacity-80 hover:bg-[#A1C398]'><MdEdit /></Link>
                </div>
            </div>
        </div>
    );
...
MyListCard.propTypes = {
    ...
    handleDeleteSpot: PropTypes.func.isRequired
};
